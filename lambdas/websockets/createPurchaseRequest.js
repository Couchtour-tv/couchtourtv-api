...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log(
      "\n************** [createPurchaseRequests.js] [31] payload Recevied:",
      postData
    )
    ...
    try {
      ...
      if (validItems) {
        ...
        const paymentIntentobjResp = await stripeInterface.paymentIntents.create(
          paymentIntentStripePayload,
          idempotencyKeyTest
        )
        console.log("\n--------------- Payment Intent Success")
        console.log(
          "\n************** [createPurchaseRequests.js] [64] reply ::",
          paymentIntentobjResp
        )
    ...
    try {
      const socket_send = await socket
        .postToConnection({
